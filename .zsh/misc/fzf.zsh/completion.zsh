...
__fzf_comprun() {
  if [[ "$(type _fzf_comprun 2>&1)" =~ function ]]; then
    _fzf_comprun "$@"
  elif [[ ${TMUX_PANE-} ]] && { [[ ${FZF_TMUX:-0} != 0 ]] || [[ ${FZF_TMUX_OPTS-} ]]; }; then
    shift
    if [[ ${FZF_TMUX_OPTS-} ]]; then
...
fzf-completion() {
  ...
  if (( ${#tokens} > 1 || fzf_force_completion )) && [[ $tail == "$trigger" ]]; then
    ...
    #cmd=$(__fzf_extract_command "$lbuf")  # needed?

    if [[ ! $cmd_word ]]; then  # $cmd?
      _fzf_exec_completion "$prefix" "$lbuf"
    elif command -v "_fzf_complete_${cmd_word}" &>/dev/null; then
      prefix="$prefix" "_fzf_complete_${cmd_word}" ${(q)lbuf}
      zle reset-prompt
    elif (( ${d_cmds[(i)$cmd_word]} <= ${#d_cmds} )); then
